    ...
    if (DateTime::createFromFormat('Y-m-d', $start_date_filter) === false || DateTime::createFromFormat('Y-m-d', $end_date_filter) === false) {
        ...
        if ($stmt_revenue = $conn->prepare($sql_revenue)) {
            ...
            if ($stmt_revenue->execute()) {
                ...
                $message .= "<div class='alert alert-danger'>خطأ في حساب الإيرادات: " . $stmt_revenue->error . "</div>";
            }
            $stmt_revenue->close();
        } else {
            $message .= "<div class='alert alert-danger'>خطأ في تحضير استعلام الإيرادات: " . $conn->error . "</div>";
        }
