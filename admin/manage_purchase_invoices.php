...
if (!$has_qty) {
  throw new Exception("كل بنود الفاتورة فارغة أو بكميات صفرية — لا يمكن استلام فاتورة فارغة.");
      ...
      while ($it = $rit->fetch_assoc()) {
        ...
        //   // if reverted and untouched (remaining == original_qty) we can reactivate and add qty
        //   $reactivate_ok = (((float)$existing_rev['remaining']) == ((float)$existing_rev['original_qty']));
        //   if ($reactivate_ok) {
        ...
        //     // ✨ تعديل: عملت متغيرات وسيطة أوضح (adj_by و bid) بدل ما أكررهم مباشرة
        //     $adj_by = $current_user_id;
        //     $bid = intval($existing_rev['id']);

        //     // if ($batches_have_sale_price) {
        //       // decide sale price: prefer item sale_price if present, else keep existing (may be null)
        //       $new_sale_price = $item_sale_price !== null ? $item_sale_price : (isset($existing_rev['sale_price']) ? $existing_rev['sale_price'] : null);

        //       if ($new_sale_price === null) {
        //         // ✨ تعديل: هنا لو sale_price ملوش قيمة، بنحدثه بـ NULL صراحةً في SQL
        //         $upb = $conn->prepare(
        //           "UPDATE batches
        //              SET qty = ?, remaining = ?, original_qty = ?, unit_cost = ?, sale_price = NULL, status = 'active', adjusted_by = ?, adjusted_at = NOW()
        //              WHERE id = ?"
        //         );
        //         if (!$upb) throw new Exception("فشل تحضير تحديث الدفعة (null sale_price): " . $conn->error);

        //         // ✨ تعديل: سلسلة الأنواع دلوقتي مظبوطة "ddddii" بعدد المتغيرات
        //         if (!$upb->bind_param("ddddii", $new_qty_total, $new_remaining, $new_original, $unit_cost, $adj_by, $bid) || !$upb->execute()) {
        ...
        //         $upb->close();
        //       // } else {
        //       //   // ✨ تعديل: لو فيه قيمة sale_price، بنمررها عادي ونستخدم bind_param "dddddii"
        //       //   $upb = $conn->prepare(
        //       //     "UPDATE batches
        //       //        SET qty = ?, remaining = ?, original_qty = ?, unit_cost = ?, sale_price = ?, status = 'active', adjusted_by = ?, adjusted_at = NOW()
        //       //        WHERE id = ?"
        //       //   );
        //       //   if (!$upb) throw new Exception("فشل تحضير تحديث الدفعة (with sale_price): " . $conn->error);

        //       //   if (!$upb->bind_param("dddddii", $new_qty_total, $new_remaining, $new_original, $unit_cost, $new_sale_price, $adj_by, $bid) || !$upb->execute()) {
        //       //     $err = $upb->error ?: $conn->error;
        //       //     $upb->close();
        ...
        //       // }
        //     } else {
        //       // ✨ تعديل: نسخة بدون sale_price، وسلسلة الأنواع "ddddii" مظبوطة
        //       $upb = $conn->prepare(
        //         "UPDATE batches
        //          SET qty = ?, remaining = ?, original_qty = ?, unit_cost = ?, status = 'active', adjusted_by = ?, adjusted_at = NOW()
        //          WHERE id = ?"
        //       );
        //       if (!$upb) throw new Exception("فشل تحضير تحديث الدفعة (no sale_price): " . $conn->error);

        //       if (!$upb->bind_param("ddddii", $new_qty_total, $new_remaining, $new_original, $unit_cost, $adj_by, $bid) || !$upb->execute()) {
        //         $err = $upb->error ?: $conn->error;
        //         $upb->close();
        //         throw new Exception("فشل تحديث الدفعة (no sale_price): " . $err);
        //       }
        //       $upb->close();
        //     }
...
//         if ($existing_rev && isset($existing_rev['id'])) {
//     // فقط نعيد التفعيل إذا الدفعة لم تتغير (لم يتم استهلاكها أو تعديلها)
//     $reactivate_ok = (((float)$existing_rev['remaining']) == ((float)$existing_rev['original_qty']));
//     if ($reactivate_ok) {
//         // 1) زد رصيد المنتج (لأن التراجع كان قد نَقَصه)
//         if (!$stmt_update_product->bind_param("di", $qty, $product_id) || !$stmt_update_product->execute()) {
//             throw new Exception('فشل تحديث المنتج: ' . $stmt_update_product->error);
//         }
...
//         // 2) أعد تفعيل الدفعة دون تغيير qty/remaining/original
//         //    فقط حدّث unit_cost و sale_price بحسب ما هو موجود في العنصر
//         if ($item_sale_price === null) {
//             $upb = $conn->prepare(
//                 "UPDATE batches
//                    SET unit_cost = ?, sale_price = NULL, status = 'active', adjusted_by = ?, adjusted_at = NOW()
//                  WHERE id = ?"
//             );
//             if (!$upb) throw new Exception("فشل تحضير تحديث الدفعة (null sale_price): " . $conn->error);
//             if (!$upb->bind_param("dii", $unit_cost, $adj_by, $bid) || !$upb->execute()) {
//                 $err = $upb->error ?: $conn->error;
//                 $upb->close();
//                 throw new Exception("فشل تحديث الدفعة (null sale_price): " . $err);
//             }
...
//             $upb = $conn->prepare(
//                 "UPDATE batches
//                    SET unit_cost = ?, sale_price = ?, status = 'active', adjusted_by = ?, adjusted_at = NOW()
//                  WHERE id = ?"
//             );
//             if (!$upb) throw new Exception("فشل تحضير تحديث الدفعة (with sale_price): " . $conn->error);
//             if (!$upb->bind_param("ddii", $unit_cost, $item_sale_price, $adj_by, $bid) || !$upb->execute()) {
//                 $err = $upb->error ?: $conn->error;
//                 $upb->close();
//                 throw new Exception("فشل تحديث الدفعة (with sale_price): " . $err);
//             }
...
//         // 3) اربط البند بالدفعة وحدد qty_received (لا نغير أي قيم كمية داخل الدفعة نفسها)
//         $new_batch_id = $bid;
//         if (!$stmt_update_item->bind_param("dii", $qty, $new_batch_id, $item_id) || !$stmt_update_item->execute()) {
//             throw new Exception('فشل ربط البند بالدفعة: ' . $stmt_update_item->error);
//         }
...
//     }
//     // إذا الدفعة reverted لم تكن untouched => سنسمح للكود أدناه بإنشاء دفعة جديدة (لا نحاول تعديل الصف الموجود)
// }
  ...
  if (isset($_POST['edit_invoice']) && isset($_POST['invoice_id'])) {
    ...
    try {
      ...
      foreach ($items_data as $it) {
        ...
        //   // حدد التكلفة المعتمدة بعد التعديل (إذا أتى new_cost استخدمه، وإلا نستخدم قيمة السجل القديمة)
